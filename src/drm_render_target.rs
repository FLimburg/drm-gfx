// #![allow(dead_code)]
use drm::control::{framebuffer, Device as ControlDevice, dumbbuffer::DumbBuffer};
...
impl RenderTarget {
    pub fn new(device: &str) -> Self {
        ...
        // Map it and grey it out.
        {
            ...
            // }
            let buf = map.as_mut();
            let line_length = disp_width as u64 * 4;
            for j in 0..disp_height {
                if j % 2 == 0 { continue;}
...
impl FramebufferTarget for RenderTarget {
    fn eat_framebuffer(&mut self, buffer: &[u32]) -> Result<(), FbWriteError> {
        ...
        if buffer.len() != pixel_count {
            panic!("Buffer length mismatch: expected {}, got {}", pixel_count, buffer.len());
        }
        // Map it and cycle colors.
        {
            let mut map = self.card
        ...
        let fb = self.card
            ...
            .expect("Could not create FB");
        self.card.set_crtc(self.crtc, Some(fb), (0, 0), &[self.connection], Some(self.mode))
            .expect("Could not set CRTC");
